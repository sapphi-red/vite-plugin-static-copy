...
- `transform` could return `null` or `undefined` as a way to tell the plugin not to copy the file, this is similar to the CopyWebpackPlugin#filter option, but it expects `transform` to return the original content in case you want it to be copied.
